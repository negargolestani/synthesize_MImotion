...
%     [dv1x, dv1y, dv1z, dv1t, dv1p] = gradient(sign(vind-V1));
%     cond5 = abs(dv1x)==2 + abs(dv1y)==2 + abs(dv1z)==2 + abs(dv1t)==2 + abs(dv1p)== 2;
    ...
    for i = 1:length(idx_list)
        ...
        if abs(V2 - v2)<err             
            sol(i,:) = [ x, y, z, theta, phi];
        end
    end
    Preds{t} = sol;
    
    if any(sol)
        C1_old = median(sol(:,1:3), 1);
...
figure
for i = 1:5
    subplot(5,1,i), plot(target(:,i)), hold on
